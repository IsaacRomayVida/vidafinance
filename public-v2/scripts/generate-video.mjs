...
async function run() {
  ...
  console.log('\nGenerating hero video with Seedance 2.0 (this can take a few minutes) …\n');
  ...
  const mp4 = path.join(VIDEO_DIR, 'hero.mp4');
  fs.writeFileSync(mp4, buf);
  console.log(`Saved ${mp4} (${(buf.length / 1024 / 1024).toFixed(1)} MB)`);

  // Poster frame for instant first paint (optional — needs ffmpeg).
  try {
    const poster = path.join(VIDEO_DIR, 'hero-poster.jpg');
    execFileSync('ffmpeg', ['-y', '-i', mp4, '-vframes', '1', '-q:v', '3', poster], { stdio: 'ignore' });
    console.log(`Saved poster ${poster}`);
  } catch {
    console.log('(ffmpeg not found — skipping poster extraction)');
  }
