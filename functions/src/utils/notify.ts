...
export async function notifyLoanEvent(event: string, data: Record<string, unknown>): Promise<void> {
  ...
  switch (event) {
    ...
    case 'loan_rejected': {
      ...
      if (employeeEmail) {
        await sendEmail(employeeEmail, 'Loan Request Update — Vida Finance', `<p>Hi ${employeeName},</p><p>Unfortunately, your loan request was not approved at this time.</p><p>— Vida Finance</p>`);
      }
      break;
    }
