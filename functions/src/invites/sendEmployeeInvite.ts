  ...
  withAuth<SendEmployeeInviteInput, SendEmployeeInviteResult>(
    ...
    async (data, auth) =>
      withErrorHandling({ functionName: 'sendEmployeeInvite', uid: auth.uid }, async () => {
        ...
        // Fail closed: an outage must not lift the brake on spamming outbound
        // invite email.
        await enforceRateLimit(`rl:sendEmployeeInvite:${auth.uid}`, 20, 60, {
          onUnavailable: 'closed',
          context: 'sendEmployeeInvite',
        });
