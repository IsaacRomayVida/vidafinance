  ...
  withAuth<AcceptInviteInput, AcceptInviteResult>(
    ...
    async (data, auth) =>
      withErrorHandling({ functionName: 'acceptInvite', uid: auth.uid }, async () => {
        // Rate limit: 20/min/uid (mutation — invite acceptance). Fail closed:
        // this is the only brake on brute-forcing another user's invite token.
        await enforceRateLimit(`rl:acceptInvite:${auth.uid}`, 20, 60, {
