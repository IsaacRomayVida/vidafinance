...
export const updateLoanStatus = onCall(
  ...
  withAuth(['ops', 'admin', 'super_admin'], async (data, auth) =>
    withErrorHandling({ functionName: 'updateLoanStatus', uid: auth.uid }, async () => {
      // Rate limit: 20/min/uid (mutation). Fail closed: this writes
      // loans/{loanId}.status, including post-disbursement transitions — an
      // outage must not lift the only brake on it. (NOT DEPLOYED — see the
      // live callable in index.ts.)
      await enforceRateLimit(`rl:updateLoanStatus:${auth.uid}`, 20, 60, {
