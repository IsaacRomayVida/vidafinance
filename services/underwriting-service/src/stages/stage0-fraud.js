 ...
 *
 * Checks:
 *   1. Behavioral biometrics (MetaMap device fingerprint + behavioral analysis)
...
async function runFraudGates(applicant, _priorResults, { logger } = {}) {
  ...
  // 1. Behavioral biometrics (MetaMap)
  if (applicant.sessionKey) {
    checks.push(
      checkBehavioralRisk(applicant.sessionKey, applicant.userId)
        .then(r => ({ key: "behavioral", result: r }))
        .catch(err => ({ key: "behavioral", result: { pass: true, skipped: true, error: err.message } }))
    );
  } else {
    checks.push(Promise.resolve({ key: "behavioral", result: { pass: true, skipped: true, reason: "no_session_key" } }));
  }
  ...
  // Decision logic
  // Behavioral very_high = hard reject
  if (data.behavioral.risk_level === "very_high") {
    return {
      pass: false,
  ...
  // Behavioral high = flag but continue
  const behavioralFlag = data.behavioral.risk_level === "high";
