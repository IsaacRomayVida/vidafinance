...
module.exports = [
  js.configs.recommended,
  {
    ...
    languageOptions: {
      ...
      globals: {
        ...globals.node,
        // `globals.node` now includes runtime globals (WHATWG URL, fetch,
        // crypto, ...) that didn't exist when the old eslintrc env was
        // written. src/metamap-signing-client.js does
        // `const { URL } = require('url')`, which is completely standard
        // Node code but trips `no-redeclare` against the newer global --
        // unset it rather than rewrite the require.
        URL: 'off',
      },
    },
    rules: {
      ...
      // idiomatic Express middleware/handlers that do `return
      // res.status(...)` on an early-exit branch and just fall off the end
      // otherwise -- behaviorally identical, flagged only for return-style
      ...
      // idiom; allow it without opening up other genuinely-empty blocks.
