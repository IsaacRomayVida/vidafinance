...
def generate_synthetic_data(n: int, rng: np.random.Generator) -> pd.DataFrame:
    ...
    df["belvo_cash_flow_avg"] = rng.lognormal(mean=9.0, sigma=0.8, size=n).clip(
        2000, 80000
    )
    df["employer_score"] = (
        rng.normal(loc=65, scale=15, size=n).clip(0, 100).astype(float)
    )
    ...
    score += (
        0.05
        * (df["cdcScore"] > 700).astype(float)
        * (df["scDiasAtraso"] < 10).astype(float)
    )
    score -= (
        0.05
        * (df["lti"] > 0.3).astype(float)
        * (df["employer_tier"] >= 4).astype(float)
    )
    score += (
        0.03
        * (df["payroll_regularity"] > 0.8).astype(float)
        * (df["employer_score"] > 70).astype(float)
    )
...
if __name__ == "__main__":
    out = os.path.join(
        os.path.dirname(__file__), "..", "models", "xgb_challenger_v2.joblib"
    )
