...
const i18n = {
  es: {
    ...
    modal_request:'Solicitar Fondos',modal_available:'Disponible',modal_amount:'Monto (MXN)',modal_term:'Plazo de Pago',modal_term_15:'15 días · 5%',modal_term_30:'30 días · 10%',modal_rate_15:'5% comisión',modal_rate_30:'10% comisión',modal_loan_amount:'Monto del préstamo',modal_fee:'Comisión',modal_total:'Pago total',modal_confirm:'Confirmar Solicitud',modal_submitting:'Enviando...',modal_exceed:'El monto excede el crédito disponible',modal_minimum:'El monto mínimo es $500 MXN',
    ...
    dash_pay_now:'Pagar ahora',dash_pay_error:'Error al generar enlace de pago. Intenta de nuevo.',
    auth_verify_email:'Verifica tu correo electrónico',auth_verify_email_sub:'Te enviamos un enlace de verificación. Revisa tu bandeja de entrada y vuelve a iniciar sesión.',auth_verify_email_btn:'Volver al inicio',auth_verify_email_msg:'Verifica tu correo electrónico. Te enviamos un enlace de verificación.',
    ...
    lp_m_widget_rate_val:'5% (15 días) · 10% (30 días)',
    lp_m_widget_term:'Plazo',
    lp_m_widget_term_val:'15 o 30 días',
    ...
    lp_m_how_3_t:'Solicita fondos',lp_m_how_3_d:'Elige el monto y plazo (15 o 30 días). Recibe fondos en 24 horas. Los pagos se descuentan de tu nómina.',
    ...
    pg_terms_3_p:'Los créditos tienen un límite máximo de $5,000 MXN, calculado en función del salario mensual declarado (hasta 30% del salario). El Empleado puede elegir un plazo de 15 días (comisión del 5%) o 30 días (comisión del 10%) sobre el monto solicitado. No existen comisiones ocultas, penalidades por pago anticipado, ni cargos adicionales. Los pagos se deducen automáticamente de la nómina del Empleado.',
    ...
    pg_press_kit_2_t:'Datos clave',pg_press_kit_2_d:'Fundada en 2024. Holding en Suiza (VIDA Holding AG). Operación en México (VIDA Finance SOFOM). Créditos de $500 a $5,000 MXN. Plazo de 15 o 30 días. Comisión del 5% (15 días) o 10% (30 días). Sin comisiones ocultas.',
  ...
  en: {
    ...
    phone_available:'Available Credit',phone_active:'Active',phone_currency:'Mexican Pesos · Via payroll',phone_repayment:'Repayment',phone_repayment_val:'15 or 30 days',phone_disbursement:'Disbursement',phone_disbursement_val:'Within 24 hrs',phone_deduction:'Deduction',phone_deduction_val:'Auto payroll',phone_utilization:'Utilization',phone_request:'Request Funds',
    ...
    modal_request:'Request Funds',modal_available:'Available',modal_amount:'Amount (MXN)',modal_term:'Repayment Term',modal_term_15:'15 days · 5%',modal_term_30:'30 days · 10%',modal_rate_15:'5% fee',modal_rate_30:'10% fee',modal_loan_amount:'Loan amount',modal_fee:'Fee',modal_total:'Total repayment',modal_confirm:'Confirm Request',modal_submitting:'Submitting...',modal_exceed:'Amount exceeds available credit',modal_minimum:'Minimum amount is $500 MXN',
    ...
    dash_pay_now:'Pay now',dash_pay_error:'Error generating payment link. Please try again.',
    auth_verify_email:'Verify your email',auth_verify_email_sub:'We sent a verification link to your inbox. Please verify and sign in again.',auth_verify_email_btn:'Back to home',auth_verify_email_msg:'Please verify your email. We sent a verification link to your inbox.',
    ...
    lp_m_widget_rate_val:'5% (15 days) · 10% (30 days)',
    lp_m_widget_term:'Term',
    lp_m_widget_term_val:'15 or 30 days',
    ...
    lp_m_how_3_t:'Request funds',lp_m_how_3_d:'Choose the amount and term (15 or 30 days). Receive funds within 24 hours. Payments are deducted from your payroll.',
    ...
    pg_terms_3_p:'Credits have a maximum limit of $5,000 MXN, calculated based on declared monthly salary (up to 30% of salary). The Employee may choose a term of 15 days (5% fee) or 30 days (10% fee) on the requested amount. There are no hidden fees, early payment penalties, or additional charges. Payments are automatically deducted from the Employee\'s payroll.',
    ...
    pg_press_kit_2_t:'Key data',pg_press_kit_2_d:'Founded in 2024. Holding in Switzerland (VIDA Holding AG). Operations in Mexico (VIDA Finance SOFOM). Credits from $500 to $5,000 MXN. Term of 15 or 30 days. Fee of 5% (15 days) or 10% (30 days). No hidden fees.',
...
function initHomeJS() {
  ...
  if (!cs) return;
  const R15 = 0.05, R30 = 0.10;
  let calcDays = 15;
  const tg = document.getElementById('termToggle');
  if (tg) {
    tg.querySelectorAll('.term-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        tg.querySelectorAll('.term-btn').forEach(x => x.classList.remove('active'));
  ...
  function upd() { const v = parseInt(cs.value); const rate = calcDays === 15 ? R15 : R30; sf.style.width = ((v - 500) / 4500 * 100) + '%'; cd.textContent = '$' + fmt(v); const tot = v * (1 + rate); const w = Math.floor(tot); const c = ((tot - w) * 100).toFixed(0).padStart(2, '0'); pd.innerHTML = '<span class="cr">$</span>' + fmt(w) + '<span class="dc">.' + c + '</span>'; }
        ...
        <div style="border:1.5px solid rgba(25,68,69,0.06);border-radius:14px;padding:36px 32px;background:var(--bg)">
          ...
          <div style="border-top:1px solid rgba(25,68,69,0.06);padding-top:24px">
            <div style="margin-bottom:20px">
              <div style="font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--t3);margin-bottom:8px">${t('lp_m_widget_available')}</div>
            ...
            <div style="margin-bottom:16px">
              <div style="font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--t3);margin-bottom:8px">${t('lp_m_widget_term')}</div>
              <div class="term-row" id="empWidgetTermToggle" style="display:flex;gap:8px"><button type="button" class="term-btn active" data-days="15" style="flex:1;padding:12px;border-radius:10px;font-size:12px"><span class="td" style="font-size:18px;display:block;line-height:1.2">15</span><span class="tl" style="font-size:9px;text-transform:uppercase;letter-spacing:1px;opacity:.6">${t('calc_days')} · 5%</span></button><button type="button" class="term-btn" data-days="30" style="flex:1;padding:12px;border-radius:10px;font-size:12px"><span class="td" style="font-size:18px;display:block;line-height:1.2">30</span><span class="tl" style="font-size:9px;text-transform:uppercase;letter-spacing:1px;opacity:.6">${t('calc_days')} · 10%</span></button></div>
            </div>
...
function initEmployeeWidget() {
  ...
  if (!salaryIn) return;
  let widgetDays = 15;
  const widgetTermToggle = document.getElementById('empWidgetTermToggle');
  if (widgetTermToggle) {
    widgetTermToggle.querySelectorAll('.term-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        widgetTermToggle.querySelectorAll('.term-btn').forEach(x => x.classList.remove('active'));
        btn.classList.add('active');
        widgetDays = parseInt(btn.dataset.days);
        update();
      });
    });
  }
  function update() {
    ...
    const rate = widgetDays === 15 ? 0.05 : 0.10;
    const fee = Math.round(capped * rate);
  ...
  document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!auth.currentUser.emailVerified) {
        ...
        errEl.textContent = t('auth_verify_email_msg');
        errEl.classList.add('show');
...
async function renderEmployerDashboard(app) {
  ...
  if (!auth.currentUser?.emailVerified) {
    app.innerHTML = `<div style="max-width:600px;margin:80px auto;text-align:center;padding:40px"><h2>${t('auth_verify_email')}</h2><p style="margin:16px 0;color:var(--t2)">${t('auth_verify_email_sub')}</p><button class="btn-primary" onclick="auth.signOut().then(()=>navigate('/login'))">${t('auth_verify_email_btn')}</button></div>`;
    return;
  }
...
async function renderEmployeeDashboard(app) {
  ...
  let modalDays=15;
  const modalTermToggle=document.getElementById('modalTermToggle');
  if(modalTermToggle){modalTermToggle.querySelectorAll('.term-btn').forEach(btn=>{btn.addEventListener('click',()=>{modalTermToggle.querySelectorAll('.term-btn').forEach(x=>x.classList.remove('active'));btn.classList.add('active');modalDays=parseInt(btn.dataset.days);const dd=new Date(Date.now()+modalDays*24*60*60*1000);document.getElementById('modalDueDate').textContent=dd.toLocaleDateString();document.getElementById('modalRateLabel').textContent=modalDays===15?'5%':'10%';updateModal();});});}
