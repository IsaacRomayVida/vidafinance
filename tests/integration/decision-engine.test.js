...
// MetaMap AML mock
function mockMetaMapAML(rfc) {
  const seed = (rfc || "").slice(-3);
  if (seed === "XXX") return {
    verificationId: `mock-aml-${rfc}`, status: "flagged",
  ...
  return {
    verificationId: `mock-aml-${rfc}`, status: "clear",
...
// MetaMap AML parseResult
function parseMetaMapAMLResult(result) {
  const confirmedAML = (result.amlScreening?.lists || []).some(h => h.matchType === "CONFIRMED");
  const fuzzyAML = (result.amlScreening?.lists || []).some(h => h.matchType === "FUZZY");
  return {
    hard_reject: confirmedAML || !!result.criminalRecords?.found,
    requires_human_review: fuzzyAML || !!result.pepCheck?.isPEP,
    pass: !confirmedAML && !result.criminalRecords?.found,
  };
}

// MetaMap KYC mock
function mockMetaMapKYC(rfc) {
  const seed = (rfc || "").slice(-3);
  if (seed === "XXX") return {
    overall: "DECLINED",
...
// MetaMap behavioral biometrics mock
function mockMetaMapBehavioral(sessionKey) {
...
function runDecisionPipeline(applicant) {
  ...
  if (currentStage === 3 && empScore.risk_tier === 1) {
    // KYC via MetaMap
    const kyc = mockMetaMapKYC(applicant.rfc);
  ...
  if (currentStage === 4) {
    ...
    // MetaMap KYC liveness check
    const kyc = mockMetaMapKYC(applicant.rfc);

    // MetaMap behavioral biometrics
    const behavioral = mockMetaMapBehavioral(applicant.behavioralSessionKey || "default");
  ...
  if (currentStage === 5) {
    ...
    // MetaMap AML check
    const amlRaw = mockMetaMapAML(applicant.rfc);
    const amlDecision = parseMetaMapAMLResult(amlRaw);
...
describe("End-to-End Integration: Decision Engine (18 Decision Paths)", () => {
  ...
  describe("Stage 4 escalation", () => {
    ...
    test("6. Stage 4 MetaMap liveness pass + Open Banking clean → approved", () => {
      ...
      expect(result.bureauDecision.escalateToStage).toBe(4);
      // Default mock RFC → MetaMap KYC APPROVED, behavioral low risk, RiskSeal pass
      expect(result.decision).toBe("approved");
      expect(result.reason).toBe("approved_stage_4");
  ...
  describe("Rejection paths", () => {
    ...
    test("14. MetaMap document verification failed (fake INE) → reject at Stage 4", () => {
      // RFC ending in XXX → MetaMap KYC DECLINED (expired/fake document)
      const result = runDecisionPipeline({
        ...BASE_APPLICANT,
        rfc: "GARL900101XXX",
        bureauResponse: {
      ...
      // Score 550 → Stage 4, RFC XXX → MetaMap KYC DECLINED + RiskSeal fail
    ...
    test("14b. Document verification failed at Stage 4 (non-XXX RFC for isolated test)", () => {
      ...
      // Simulate MetaMap KYC returning DECLINED but RiskSeal passing
      const applicant = {
        ...BASE_APPLICANT,
        bureauResponse: {
          cdc: { score: 550, diasAtraso: 0, carteraVencida: false },
      ...
      // Directly test MetaMap KYC mock for XXX seed
      const kycResult = mockMetaMapKYC("GARL900101XXX");
      expect(kycResult.overall).toBe("DECLINED");
      expect(kycResult.idValidation.pass).toBe(false);
    });

    test("15. MetaMap AML hit → flag for human review at Stage 5", () => {
      // RFC ending in YYY → MetaMap fuzzy AML match → human review
      const result = runDecisionPipeline({
        ...BASE_APPLICANT,
        rfc: "GARL900101YYY",
        bureauResponse: {
    ...
    test("15b. Confirmed AML (OFAC) → hard reject at Stage 5", () => {
      // RFC ending in XXX → MetaMap confirmed OFAC match → hard reject
      const result = runDecisionPipeline({
        ...BASE_APPLICANT,
        rfc: "GARL900101XXX",
        bureauResponse: {
  ...
  describe("Edge cases", () => {
    ...
    test("16. MetaMap API timeout → fallback to local model (graceful degradation)", () => {
      ...
      const result = runDecisionPipeline({
        ...BASE_APPLICANT,
        rfc: "GARL900101YYY", // → MetaMap KYC MANUAL_REVIEW
        bureauResponse: {
          cdc: { score: 550, diasAtraso: 0, carteraVencida: false },
          bdc: {},
        },
      ...
      // Score 550 → Stage 4, RFC YYY → MetaMap KYC MANUAL_REVIEW
      // RiskSeal YYY → score 38 (high risk, but >= 30 so no RiskSeal escalation)
      // Behavioral passes, open banking clean — but MetaMap KYC not APPROVED/DECLINED
      // Falls through to "other Stage 4 failures" → Stage 5
      expect(result.stagesReached).toContain("stage_4_verification");
      // With YYY: RiskSeal score=38 ≥ 30, behavioral passes, but KYC pass check...
      // Actually YYY RFC → RiskSeal score 38 (pass: score >= 30 → true)
      // But MetaMap KYC overall is MANUAL_REVIEW, liveness passes
  ...
  describe("Mock API clients — seed-based determinism", () => {
    ...
    test("MetaMap AML: XXX → hard reject, YYY → human review, default → clean", () => {
      const xxx = parseMetaMapAMLResult(mockMetaMapAML("RFC123XXX"));
      expect(xxx.hard_reject).toBe(true);
      expect(xxx.pass).toBe(false);

      const yyy = parseMetaMapAMLResult(mockMetaMapAML("RFC123YYY"));
      expect(yyy.hard_reject).toBe(false);
      expect(yyy.requires_human_review).toBe(true);

      const clean = parseMetaMapAMLResult(mockMetaMapAML("RFC123ABC"));
      expect(clean.hard_reject).toBe(false);
      expect(clean.requires_human_review).toBe(false);
      expect(clean.pass).toBe(true);
    });

    test("MetaMap KYC: XXX → declined, YYY → manual review, default → approved", () => {
      expect(mockMetaMapKYC("RFC123XXX").overall).toBe("DECLINED");
      expect(mockMetaMapKYC("RFC123YYY").overall).toBe("MANUAL_REVIEW");
      expect(mockMetaMapKYC("RFC123ABC").overall).toBe("APPROVED");
    });

    test("MetaMap Behavioral: BAD → very_high, MED → high, default → low (pass)", () => {
      expect(mockMetaMapBehavioral("sessionBAD").pass).toBe(false);
      expect(mockMetaMapBehavioral("sessionMED").pass).toBe(false);
      expect(mockMetaMapBehavioral("sessionOK").pass).toBe(true);
    });
